...
_folder_mapping = {
    'sub-Jenkins': 'traintest',
    }


def get_args_parser():
    parser = argparse.ArgumentParser('Consolidate data in multiple files into a single file', add_help=False)
    parser.add_argument('--data_dir',default="sub-Jenkins",type=str, choices=["sub-Jenkins"], help='Data directory')
    return parser
...
if __name__ == '__main__':
    ...
    ds.push_to_hub("eminorhan/mc-maze", _folder_mapping[os.path.basename(args.data_dir)], split="train", token=True)
